...
class LongShort {
    constructor(API_KEY, API_SECRET, PAPER){
      ...
      this.allStocks = require('./stocks.json').stocks;//['DOMO', 'TLRY', 'SQ', 'MRO', 'AAPL', 'GM', 'SNAP', 'SHOP', 'SPLK', 'BA', 'AMZN', 'SUI', 'SUN', 'TSLA', 'CGC', 'SPWR', 'NIO', 'CAT', 'MSFT', 'PANW', 'OKTA', 'TM', 'GS', 'BAC', 'MS', 'TWLO', 'QCOM'];
  ...
  getTotalPrice(stocks){
    ...
    stocks.forEach(async (stock) => {
      proms.push(new Promise(async (resolve, reject) => {
        const bars = this.alpaca.getBarsV2(stock, {limit: 1, timeframe: this.alpaca.newTimeframe(1, this.alpaca.timeframeUnit.MIN),}); //.then((resp) => {
        const gotBars = [];
        for await (let b of bars) {
            gotBars.push(b);
        }

            //console.log(stock, gotBars);
          resolve(gotBars[0].ClosePrice);
        //}).catch((err) => {console.log(err.error);});
      }));
    });
    return proms;
    ...
    getPercentChanges(allStocks){
        ...
        allStocks.forEach((stock) => {
            promStocks.push(new Promise(async (resolve, reject) => {
                const bars = this.alpaca.getBarsV2(stock.name, {limit: length, timeframe: this.alpaca.newTimeframe(1, this.alpaca.timeframeUnit.MIN)})//.then((resp) => {
                const gotBars = [];
                for await (let b of bars) {
                    gotBars.push(b);
                }
                    ...
                    stock.pc  = (gotBars[length - 1].ClosePrice - gotBars[0].OpenPrice) / gotBars[0].OpenPrice;
                    //console.log(stock.pc);
                //}).catch((err) => {console.log(err.error);});
                resolve();
            }));
        });
